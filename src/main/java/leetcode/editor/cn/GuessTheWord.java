//这是一个 交互式问题 。 
//
// 我们给出了一个由一些 不同的 单词组成的列表 wordlist ，对于每个 wordlist[i] 长度均为 6 ，这个列表中的一个单词将被选作 
//secret 。 
//
// 你可以调用 Master.guess(word) 来猜单词。你所猜的单词应当是存在于原列表并且由 6 个小写字母组成的类型 string 。 
//
// 此函数将会返回一个 integer ，表示你的猜测与秘密单词 secret 的准确匹配（值和位置同时匹配）的数目。此外，如果你的猜测不在给定的单词列表中，
//它将返回 -1。 
//
// 对于每个测试用例，你有 10 次机会来猜出这个单词。当所有调用都结束时，如果您对 Master.guess 的调用在 10 次以内，并且至少有一次猜到 
//secret ，将判定为通过该用例。 
//
// 
//
...
// numguesses == 10 
// 
// Related Topics 数组 数学 字符串 博弈 交互 👍 109 👎 0
...
public class GuessTheWord {
    ...
    public static void main(String[] args) {
        Solution solution = new GuessTheWord().new Solution();
    }
